...
### 1. Whitelisted IPs
A valid IP must be included in `configured-ips`. Whitelisted IPs will be granted admin permissions. To configure this option, follow these [instructions](/docs/community/com-config-auth/com-config-auth-home#configuring-featurebase).
### 2. auth-token flag
A valid JWT must be passed to the `auth-token` flag for any ingester. The user must have admin permissions. The token may be obtained by following these [instructions](/docs/community/com-config-auth/com-config-auth-home#how-to-get-auth-token).
...
| auth-token                     | string  | JWT authentication token obtained by following these [instructions](/docs/community/com-config-auth/com-config-auth-home#how-to-get-auth-token) |
