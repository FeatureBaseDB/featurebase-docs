...
When authentication is enabled, gRPC endpoints can be authenticated by passing a valid JWT token in the `Authorization` header. The token may be obtained by following these [instructions](/docs/community/com-config-auth/com-config-auth-home#how-to-get-auth-token).
